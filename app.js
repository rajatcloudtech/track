...
function rDash() {
  ...
  document.getElementById('d-glist').innerHTML = !tg.length
    ...
    : tg.slice(0, 4).map(g =>
        `<div class="gi ${g.done ? 'done' : ''}" onclick="toggleGoal('${g.id}','dash')">
          <div class="gck">${g.done ? '✓' : ''}</div>
          <div class="gt">${esc(g.text)}</div>
          <div class="pill ${CAT_CLASS[g.cat] || 'pd'}">${g.cat}</div>
        </div>`).join('')
...
function rInsights() {
  ...
  document.getElementById('d-ins').innerHTML = ins.map(i =>
    `<div style="display:flex;gap:12px;align-items:flex-start;margin-bottom:8px;padding:11px;background:var(--bg3);border:1px solid var(--b1);border-radius:var(--rs)">
      <div style="font-size:17px;color:${i.c};flex-shrink:0">${i.i}</div>
      <div><div style="font-size:13px;font-weight:600;margin-bottom:2px">${i.ti}</div>
      <div style="font-size:12px;color:var(--txt2);line-height:1.6">${i.b}</div></div>
    </div>`).join('');
}
...
function categoryPicker() {
  ...
  return `<div class="cat-wrap">${cats.map(c => {
    const cnt = c === 'All' ? all.length : all.filter(x => x.category === c).length;
    return `<button class="cat-btn${c === activeCat ? ' cat-on' : ''}" onclick="selectCat('${c.replace(/'/g, "\\'")}')">${esc(c)}<span class="cat-cnt">${cnt}</span></button>`;
  }).join('')}</div>`;
}
...
function rCards(cards, b) {
  ...
  b.innerHTML = picker + `
    <div class="fc" id="fc" onclick="this.classList.toggle('flip')">
      <div class="fci">
        <div class="ff">
          <div class="fcat">${esc(c.category || 'SAP MM')}</div>
          <div class="fq">${esc(c.question)}</div>
          <div class="fhint">Tap to reveal answer</div>
        </div>
        <div class="fb">
          <div class="fcat">Answer</div>
          <div class="fans">${esc(c.answer)}</div>
          ${c.table ? '<div style="margin-top:10px">' + c.table.split(',').map(t => `<span class="ftag">${t.trim()}</span>`).join('') + '</div>' : ''}
        </div>
      </div>
    </div>
    <div class="cnav">
      <div class="cnum">${curCard + 1} / ${filtered.length}</div>
      <div style="display:flex;gap:8px">
        <button class="btn btn-g btn-s" onclick="prevCard(${filtered.length})">←</button>
        <button class="btn btn-g btn-s" onclick="nextCard(${filtered.length})">→</button>
        <button class="btn btn-g btn-s" onclick="shuffleCards()">⇌</button>
      </div>
    </div>
    <div style="text-align:center;font-size:11px;color:var(--txt3);padding:0 16px 16px">${filtered.length} cards · tap to flip</div>`;
}
...
function rSQL(tips, b) {
  ...
  b.innerHTML = '<div style="padding:12px 16px">' + tips.map(t => `
    <div class="sqltip">
      <div class="sth"><div class="sttl">${esc(t.title)}</div><div class="lv l${(t.level || 'b')[0]}">${t.level || 'basic'}</div></div>
      <div class="stdesc">${esc(t.desc)}</div>
      <div style="position:relative">
        <div class="code">${t.code}</div>
        <button class="cpbtn" onclick="cp(\`${(t.raw || '').replace(/\\/g, '\\\\').replace(/`/g, '\\`')}\`)">copy</button>
      </div>
...
function rTables(tables, b) {
  ...
  b.innerHTML = `<div style="padding:12px 16px"><div class="card" style="overflow-x:auto;padding:0">
    <table class="rtable">
      <thead><tr><th>Table</th><th>Description &amp; When to Use</th></tr></thead>
      <tbody>${tables.map(t => `
        <tr>
...
function rMvt(mvt, b) {
  ...
  b.innerHTML = `<div style="padding:12px 16px"><div class="card" style="overflow-x:auto;padding:0">
    <table class="rtable">
      <thead><tr><th>Mvt</th><th>Name &amp; Notes</th></tr></thead>
      <tbody>${mvt.map(m => `
        <tr>
...
function rGoals() {
  ...
  document.getElementById('glist').innerHTML = !tg.length
    ...
    : tg.map(g =>
        `<div class="gi ${g.done ? 'done' : ''}" onclick="toggleGoal('${g.id}','goals')">
          <div class="gck">${g.done ? '✓' : ''}</div>
          <div class="gt">${esc(g.text)}</div>
          <div class="pill ${CAT_CLASS[g.cat] || 'pd'}">${g.cat}</div>
          <button class="del" onclick="event.stopPropagation();deleteGoal('${g.id}')">×</button>
        </div>`).join('');

  document.getElementById('suglist').innerHTML = SUGGESTED.map((s, i) =>
    `<div class="gi" style="cursor:default">
      <div class="pill ${CAT_CLASS[s.c] || 'pd'}" style="flex-shrink:0">${s.c}</div>
      <div class="gt" style="font-size:13px">${esc(s.t)}</div>
      <button class="btn btn-g btn-s" onclick="addSuggested(${i})">+</button>
    </div>`).join('');
}
...
function rLog() {
  ...
  ll.innerHTML = db.logs.map(l => {
    ...
    return `<div class="le ${l.type}">
      <div class="lh">
        <div class="lt">${esc(l.title)}</div>
        <div style="display:flex;align-items:center;gap:6px">
          <div class="ld">${fmtDate(l.date)}</div>
          <button class="del" onclick="deleteLog('${l.id}')">×</button>
        </div>
      </div>
      <div class="lb2">${esc(l.body)}</div>
      ${tags.length ? '<div class="ltags">' + tags.map(t => `<span class="ltag">${t}</span>`).join('') + '</div>' : ''}
    </div>`;
  }).join('');
}
...
function rSkillGroup(elId, group, dotClass) {
  document.getElementById(elId).innerHTML = Object.entries(group).map(([name, val]) =>
    `<div class="ski">
      <div class="skn">${name}</div>
      <div class="dots">${[1,2,3,4,5].map(i =>
        `<div class="dot ${val >= i ? dotClass : ''}" onclick="setSkill('${elId}','${name}',${i})">${i}</div>`
      ).join('')}</div>
    </div>`).join('');
}
...
function rActivity() {
  ...
  document.getElementById('wrow').innerHTML = wdates.map((date, i) =>
    `<div class="wd ${(db.heatmap[date] || 0) > 0 ? 'on' : ''} ${date === today() ? 'now' : ''}" onclick="toggleWeekDay('${date}')">
      <div class="wdot"></div>${days[i]}
    </div>`).join('');
  ...
  pr.innerHTML = db.reviews.map(r =>
    `<div class="card">
      <div style="display:flex;justify-content:space-between;margin-bottom:10px">
        <div class="ct" style="margin:0">${fmtDate(r.date)}</div>
        <button class="del" onclick="deleteReview('${r.id}')">×</button>
      </div>
      ${fields.filter(f => r[f.k]).map(f =>
...
function rRoadmap() {
  ...
  document.getElementById('phsl').innerHTML = phases.map(p => {
    ...
    return `<div class="psl">
      <div class="psl-l" style="color:${p.c}">${p.l}</div>
      <input type="range" min="0" max="100" value="${v}" oninput="setPhase('${p.k}',this.value,this.nextElementSibling)">
      <div class="ppct">${v}%</div>
    </div>
    <div class="pb" style="margin-bottom:12px"><div class="pf" style="width:${v}%;background:${p.c}"></div></div>`;
  }).join('');
}
